...
# set up grid of 0-999x0-999, init OFF
grid = Array.new(1000) {Array.new(1000) {"0"} }
# turn each input item into an array
input.each do |z|
	if z.match /on/
		a = "on"
	elsif z.match /off/
...
instructions.each do |i|
	y_from = i[1]	# 1st coord, 1st num
	y_to = i[3]		# 2nd coord, 1st num
	x_from = i[2]	# 1st coord, 2nd num
	x_to = i[4]		# 2nd coord, 2nd num
	
	light_selection = grid[y_from..y_to]
	light_selection.each do |lights|
			lights[x_from..x_to].each do |light|
				z = light.to_i
				case i[0]
					when "on"
					zz = z+1
...
brightness = 0
grid_sum = grid.flatten
grid_sum.each do |cell|
		zzzz = cell.to_i
		brightness+=zzzz
end
